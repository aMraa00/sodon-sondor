...
export default function DoctorAppointments() {
  ...
  const handleStatus = async (id, status) => {
    try { await appointmentService.updateStatus(id, { status }); toast.success('Төлөв шинэчлэгдлээ'); load(); }
  ...
  return (
    <div className="page-container py-6">
      ...
      {loading ? <SkeletonList count={6} /> : appointments.length === 0 ? (
        <Empty icon="📅" title="Цаг захиалга байхгүй байна" />
      ) : (
          ...
          {appointments.map((appt) => (
            <div key={appt._id} className="bg-white dark:bg-slate-900 rounded-2xl border border-slate-200 dark:border-slate-800 p-4 shadow-soft">
              <div className="flex items-start gap-3">
                <div className="w-12 h-12 brand-gradient rounded-xl flex flex-col items-center justify-center text-white flex-shrink-0 text-xs">
                ...
                <div className="flex gap-1">
                  {appt.status === 'pending' && (
                    <button onClick={() => handleStatus(appt._id, 'confirmed')} className="p-1.5 text-blue-600 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg" title="Батлах">
                      <CheckCircle className="w-5 h-5" />
                    </button>
                  )}
                  {(appt.status === 'confirmed' || appt.status === 'pending') && (
                    <>
                      <button onClick={() => handleStatus(appt._id, 'completed')} className="p-1.5 text-green-600 hover:bg-green-50 dark:hover:bg-green-900/20 rounded-lg" title="Дуусгах">
                        <CheckCircle className="w-5 h-5" />
                      </button>
                      <button onClick={() => handleStatus(appt._id, 'cancelled')} className="p-1.5 text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg" title="Цуцлах">
